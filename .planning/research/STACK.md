...
npm install -D typescript@^6.0.0 vite@^8.0.0 vitest@^4.1.0 @types/react@^19.0.0 jsdom@^29.0.0
...
**For WeWeb JSON parsing (core work):**
...
**For CSS variable extraction (design tokens):**
...
- Classify by value pattern:
  - Matches `font-weight font-size/line-height var(--ww-default-font-family...)` → typography token
  - Matches `#RRGGBB` or `#RRGGBBAA` → color token
  - Matches pure `{N}px` → spacing/size token
...
These are empirically verified against the sample export ZIP (`f4f96557...216.zip`):
...
**HTML shell structure** (confirmed):
...
**JSON data file structure** (confirmed top-level keys):
```
cacheVersion, page, sections, wwObjects, collections, variables, workflows, formulas, libraryComponents
```
...
---
*Stack research for: Ship Studio plugin — WeWeb export to coding brief*
*Researched: 2026-03-24*
